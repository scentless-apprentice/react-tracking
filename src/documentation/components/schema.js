...
const generator = {
  generate(schemaData, propName = '', parentName = '') {
  ...
  buildPropertiesSegment(properties = [], parentName = '') {
    return Object.keys(properties)
      .map(prop => this.generate(properties[prop], prop, parentName))
      .join('\n\n');
  },

  init(schemaData) {
    return this.generate(schemaData, 'datalayer');
  },
};

module.exports = function schemaGenerator(options, { schemaData }) {
